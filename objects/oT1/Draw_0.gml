...
draw_set_color(c_white);
draw_text_ext(190,10,"Press 2 or click button below to request for an extraction. Then click on desired spot to set extraction area. The helicopter will only land if there is one survivor remaining. The zone will attract attention from all zombies and humans, so immediately setting the zone is not always the smartest move!",15,700);
